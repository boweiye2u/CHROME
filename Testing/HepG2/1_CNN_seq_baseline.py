...
metric_save_path = "/mnt/nfs/bowei/CHROME/model/ChIP-seq/CNN_seq_baseline/"
...
class SpecificDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(dna_seq, dtype=torch.float32), torch.tensor(label_vector, dtype=torch.float32)
...
def test_model(test_files, model_path, seq_length=5000, embed_dim=512, n_labels=751):
    device = torch.device("cuda:5" if torch.cuda.is_available() else "cpu")
    model = backbone_CNN(nclass=n_labels, seq_length=seq_length, embed_length=embed_dim).to(device)
...
if __name__ == "__main__":
    ...
    test_model(test_files, best_model_path)
