...
from layers import EVO2_GAT_eQTL
...
class EQTLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        entry = self.data[idx]
        x = torch.tensor(entry["GAT_EVO_concat_embedding"], dtype=torch.float).squeeze(0)
        y = torch.tensor(int(np.argmax(entry["label"])), dtype=torch.long)
        return x, y

with open("/mnt/nfs/bowei/CHROME/data/eQTL/gm_with_evo2_embeddings.pkl", "rb") as f:
    gm_data = [pickle.load(f) for _ in range(314)]
...
for fold, (train_idx, val_idx) in enumerate(kf.split(gm_data, label_indices)):
    print(f"\n🌀 Fold {fold+1} | Class counts in val set: {Counter([label_indices[i] for i in val_idx])}")
    ...
    model_path = f"/mnt/nfs/bowei/CHROME/model/eQTL/EVO2_GAT_embedding/gm_model_fold_{fold+1}.pt"
    torch.save(model.state_dict(), model_path)
    print(f"✅ EVO2 GAT Model fold {fold+1} saved to {model_path}")
    ...
    metrics_path = f"/mnt/nfs/bowei/CHROME/model/eQTL/EVO2_GAT_embedding/gm_metrics_fold_{fold+1}.txt"
...
avg_path = "/mnt/nfs/bowei/CHROME/model/eQTL/EVO2_GAT_embedding/gm_metrics_average.txt"
