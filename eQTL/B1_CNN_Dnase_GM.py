...
class EQTLDataset(Dataset):
    ...
    def __getitem__(self, idx):
        entry = self.data[idx]
        x = torch.tensor(entry["CNN_TSS_VAR_concat"], dtype=torch.float).squeeze(0)
        y = torch.tensor(int(np.argmax(entry["label"])), dtype=torch.long)
        return x, y

with open("/mnt/nfs/bowei/CHROME/data/eQTL/gm_with_embeddings_dnase.pkl", "rb") as f:
...
for fold, (train_idx, val_idx) in enumerate(kf.split(gm_data, label_indices)):
    ...
    model = CNN_Baseline_eQTL().to(device)
    ...
    model_path = f"/mnt/nfs/bowei/CHROME/model/eQTL/Dnase_CNN_embedding/gm_model_fold_{fold+1}.pt"
    torch.save(model.state_dict(), model_path)
    print(f"✅ Model for fold {fold+1} saved to {model_path}")

    metrics_path = f"/mnt/nfs/bowei/CHROME/model/eQTL/Dnase_CNN_embedding/gm_metrics_fold_{fold+1}.txt"
...
avg_path = "/mnt/nfs/bowei/CHROME/model/eQTL/Dnase_CNN_embedding/gm_metrics_average.txt"
