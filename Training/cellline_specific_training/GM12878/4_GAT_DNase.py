...
def build_graph_data(chunk_path):
    ...
    for entry in data_chunk:
        ...
        node_features = node_features.permute(0, 1, 2)  
...
def train_and_validate(model, criterion, optimizer, scheduler, train_files, val_files, device, pretrained_cnn, log_path, batch_size=16, sample_fraction=1):
    ...
    with open(log_path, "w") as log:
        ...
        for epoch in range(15):
            ...
            for file_path in sampled_train_files:
                file_count += 1
                print(f"Training enhanced GAT with Dnase frozen CNN (graph label) with {file_path}...")
                ...
                if file_count % 20 == 0:
                    ...
                    with torch.no_grad():
                        for val_file in val_files:
                            ...
                            val_loader = DataLoader(GraphDataset(val_graphs), batch_size=batch_size, shuffle=False, drop_last=True)
                            for val_batch in val_loader:
                                val_batch.x = val_batch.x.permute(0, 1, 2)
            ...
            current_lr = optimizer.param_groups[0]['lr']
            log.write(f"Current LR: {current_lr:.2e}\n")
            log.flush()
...
def train_model(data_dir, model_save_path):
    ...
    embed_dim = 512
    num_classes = 751
    pretrained_cnn_dnase = backbone_CNN_Dnase(nclass=num_classes, seq_length=5000, embed_length=embed_dim)
    state_dict = torch.load(
    "/mnt/nfs/bowei/CHROME/model/cellline_specific/GM/CNN_DNase_baseline/best_model.pt",
    map_location="cuda:7"
    )
    pretrained_cnn_dnase.load_state_dict(state_dict)
    pretrained_cnn_dnase = pretrained_cnn_dnase.to(device)
    pretrained_cnn_dnase.eval()
    model = GAT_DNase( embed_dim=embed_dim, num_classes=num_classes, pretrained_cnn=pretrained_cnn_dnase)
    ...
    train_and_validate(model, criterion, optimizer, scheduler, train_files, val_files, device, pretrained_cnn_dnase, log_path)

if __name__ == "__main__":
    data_dir = "/mnt/nfs/bowei/CHROME/data/cellline_specific/GM/"
    model_save_path = "/mnt/nfs/bowei/CHROME/model/cellline_specific/GM/GAT_DNase/"
