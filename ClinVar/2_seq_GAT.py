...
from layers import GAT_ClinVar
...
INPUT_DIR  = "/mnt/nfs/bowei/CHROME/data/ClinVar/GAT_seq_only/"
SAVE_DIR   = "/mnt/nfs/bowei/CHROME/model/ClinVar/GAT_seq_only_MLP/"
os.makedirs(SAVE_DIR, exist_ok=True)
CV_OUT_DIR = "/mnt/nfs/bowei/CHROME/model/ClinVar/"
CV_TAG     = "GAT_seq_only_MLP"
...
class ClinVarGATDataset(Dataset):
    def __init__(self, entries): self.entries = entries
    def __len__(self): return len(self.entries)
    def __getitem__(self, idx):
        e = self.entries[idx]
        x = torch.tensor(e[EMBED_KEY], dtype=torch.float).squeeze(0)  # (1024,)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(data, labels), start=1):
    ...
    train_loader = DataLoader(ClinVarGATDataset(train_entries), batch_size=BATCH_SIZE, shuffle=True,
                              num_workers=2, pin_memory=True)
    val_loader   = DataLoader(ClinVarGATDataset(val_entries), batch_size=BATCH_SIZE, shuffle=False,
                              num_workers=2, pin_memory=True)

    model     = GAT_ClinVar().to(DEVICE)
    optimizer = AdamW(model.parameters(), lr=LR, weight_decay=WD)
    criterion = CrossEntropyLoss()

    best_val_loss = float("inf")
    best_ckpt_path = os.path.join(SAVE_DIR, f"ClinVar_gat_mlp_seqonly_fold{fold}.pt")
