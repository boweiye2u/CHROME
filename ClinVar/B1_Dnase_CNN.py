...
from layers import CNN_Baseline_ClinVar 
...
INPUT_DIR  = "/mnt/nfs/bowei/CHROME/data/ClinVar/enriched_chunks_GAT_CNN/"
SAVE_DIR   = "/mnt/nfs/bowei/CHROME/model/ClinVar/CNN_baseline_Dnase_MLP/"
os.makedirs(SAVE_DIR, exist_ok=True)
CV_OUT_DIR = "/mnt/nfs/bowei/CHROME/model/ClinVar/"
CV_TAG     = "CNN_baseline_Dnase_MLP"
...
for fold, (train_idx, val_idx) in enumerate(skf.split(data, labels), start=1):
    ...
    model     = CNN_Baseline_ClinVar(input_dim=512, hidden_dim=256, output_dim=2, dropout=0.3).to(DEVICE)
    optimizer = AdamW(model.parameters(), lr=LR, weight_decay=WD)
    criterion = CrossEntropyLoss()

    best_val_loss = float("inf")
    best_ckpt_path = os.path.join(SAVE_DIR, f"ClinVar_cnn_mlp_fold{fold}.pt")
