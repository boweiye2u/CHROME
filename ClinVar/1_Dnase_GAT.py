...
INPUT_DIR  = "/mnt/nfs/bowei/CHROME/data/ClinVar/GAT_DNase/"
SAVE_DIR   = "/mnt/nfs/bowei/CHROME/model/ClinVar/GAT_DNase/"
...
class ClinVarGATDataset(Dataset):
    ...
    def __getitem__(self, idx):
        e = self.entries[idx]
        x = torch.tensor(e[EMBED_KEY], dtype=torch.float).squeeze(0)  
...
for fold, (train_idx, val_idx) in enumerate(skf.split(data, labels), start=1):
    ...
    best_ckpt_path = os.path.join(SAVE_DIR, f"ClinVar_gat_mlp_fold{fold}.pt")
