...
from layers import EVO_Baseline_ClinVar
from cv_saver import CVFoldSaver

EMB_DIR = "/mnt/nfs/bowei/CHROME/data/ClinVar/enriched_chunks_EVO2_GAT_embeddings/"
OUT_DIR = "/mnt/nfs/bowei/CHROME/model/ClinVar/EVO_center_MLP/"
os.makedirs(OUT_DIR, exist_ok=True)
CV_OUT_DIR = "/mnt/nfs/bowei/CHROME/model/ClinVar/"
CV_TAG     = "EVO_center_MLP"
...
def train_one_fold(fold, X, y, outdir):
    ...
    train_ds = EmbDataset(X[train_idx], y[train_idx])
    val_ds   = EmbDataset(X[val_idx],   y[val_idx])
    train_loader = DataLoader(train_ds, batch_size=BATCH_SIZE, shuffle=True)
    val_loader   = DataLoader(val_ds,   batch_size=BATCH_SIZE, shuffle=False)

    model = EVO_Baseline_ClinVar(input_dim=IN_DIM, output_dim=2).to(DEVICE)
    ...
    for ep in range(1, EPOCHS+1):
        ...
        print(f"[Center MLP] Fold {fold+1} Ep {ep:02d} | Train {tr_loss:.4f} | Val {va_loss:.4f} | "
        ...
        if va_loss + MIN_DELTA < best_val:
            ...
            no_imp += 1
            if no_imp >= PATIENCE:
                print(f"[Center MLP] ⏹️ Early stop at epoch {ep}")
                break

    if best_state is not None: model.load_state_dict(best_state)

    ckpt = os.path.join(outdir, f"EVO_center_MLP_fold_{fold+1}.pt")
    torch.save(model.state_dict(), ckpt)
    print(f"[Center MLP] ✅ Saved → {ckpt}")
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "EVO_center_MLP_metrics_average.txt"), "w") as f:
        f.write(f"Avg AUROC: {np.mean(aurocs):.4f}\n")
    print("\n[Center MLP] 📈 Avg AUROC:", f"{np.mean(aurocs):.4f}" if aurocs else "nan")
